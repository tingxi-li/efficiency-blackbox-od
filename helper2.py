...
class YOLOModel(UnifiedModel):
    ...
    def label_to_name(self, labels):
        return [self.class_names[label.item()] for label in labels]
...
if __name__ == "__main__":
    ...
    for i in range(len(subset_ids)):
        print(f"Processed image: {subset_ids[i]}")
        print("YOLOv5 labels:", yolov5su.label_to_name(yolov5su_results[i]["labels"][:10]))
        print("YOLOv8 labels:", yolov8.label_to_name(yolov8_results[i]["labels"][:10]))
        print("RT-DETR labels:", rtdetr.label_to_name(rtdetr_results[i]["labels"][:10]))
